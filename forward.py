...
def load_lexicon(lexicon_file):
    ...
    with open(lexicon_file, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            Word, Word_ID, Lexicon_ID = line.strip().split(',')
            lexicon[Word] = (int(Word_ID), int(Lexicon_ID))  # Store word_id and lexicon_id as a tuple
    return lexicon
...
def process_document(doc, lexicon, max_frequency=255):
    ...
    title, text, url, authors, timestamp, tags = doc
    ...
    for word, freq in word_frequency.items():
        word_id, lexicon_id = lexicon.get(word, (None, None))
        if word_id is not None:
            ...
            word_info.append((word_id, lexicon_id, bit_array))
...
def process_data(data_file, lexicon_file, max_frequency=255):
    ...
    with open(data_file, 'r', encoding='utf-8') as f:
        ...
        for document_id, row in enumerate(reader, 1):  # Start document_id from 1
            if len(row) == 6:  # Ensure we have exactly 6 columns
                title, text, url, authors, timestamp, tags = row
                doc = [title, text, url, authors, timestamp, tags]
                word_info = process_document(doc, lexicon, max_frequency)
                
                # Collect the output: document ID + word ID + lexicon ID + bit array
                output_line = [str(document_id)] + [f"{word_id}:{lexicon_id}:{bit_array}" for word_id, lexicon_id, bit_array in word_info]
                output_data.append('\t'.join(output_line))
            else:
                print(f"Skipping line {document_id} due to incorrect number of columns.")
...
def save_output(output_file, output_data):
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        # Write header
        f.write('document_id,word_id:lexicon_id:bit_array\n')
...
lexicon_file = 'lexicon.csv'  # File containing lexicon (word, word_id, lexicon_id)
